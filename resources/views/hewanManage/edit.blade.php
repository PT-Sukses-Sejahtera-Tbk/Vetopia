                    ...
                    <form method="POST" action="{{ route('hewan.update', $hewan->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            @if (auth()->user()->hasRole('user'))
                                <input type="text" value="{{ $hewan->pemilik->name }}" disabled
                                    class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 shadow-sm">
                                <input type="hidden" name="user_id" value="{{ $hewan->user_id }}">
                            @else
                                <select name="user_id" id="user_id" required
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            {{ old('user_id', $hewan->user_id) == $user->id ? 'selected' : '' }}>
                                            {{ $user->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="nama" id="nama" value="{{ old('nama', $hewan->nama) }}"
                                required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @error('nama')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="ras" id="ras" value="{{ old('ras', $hewan->ras) }}"
                                required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @error('ras')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="umur" id="umur" value="{{ old('umur', $hewan->umur) }}"
                                required min="0"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            @error('umur')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
