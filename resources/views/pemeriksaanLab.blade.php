...
@section('container')
    <div class="my-10 text-gray-900 font-sans text-center">
        <p class="font-medium text-2xl">Form Pendaftaran Pemeriksaan Lab</p>
        <p class="text-sm font-normal">Lengkapi informasi kesehatan hewan Anda untuk pemeriksaan laboratorium</p>
    </div>
    ...
    <div class="max-w-xl mx-auto px-4 pb-20">
        <div class="bg-white rounded-3xl border border-gray-200 p-8 shadow-sm">
            <form id="bookingForm" action="{{ route('pemeriksaan.lab.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-8">
                    ...
                    <div class="mb-4">
                        ...
                        <span class="error-message text-sm hidden" id="error-hewan_id" style="color: #ef4444;">Pilih hewan terlebih dahulu</span>
                    </div>
                ...
                {{-- Section 2: Detail Pemeriksaan Lab --}}
                <div class="mb-8">
                    <div class="flex items-center gap-2 mb-6">
                        <div class="w-8 h-8 bg-vetopia-green rounded-lg flex items-center justify-center">
                            {{-- Icon SVG Lab --}}
                        ...
                        <h3 class="text-lg font-semibold">Detail Pemeriksaan</h3>
                    </div>

                    <div class="mb-4">
                        <label for="jenis_pemeriksaan" class="block text-sm font-medium text-gray-700 mb-2">Jenis Pemeriksaan</label>
                        <select id="jenis_pemeriksaan" name="jenis_pemeriksaan" class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                            <option value="">Pilih Layanan Lab</option>
                            <option value="Cek Darah Lengkap" {{ old('jenis_pemeriksaan') == 'Cek Darah Lengkap' ? 'selected' : '' }}>Cek Darah Lengkap</option>
                            <option value="Urine" {{ old('jenis_pemeriksaan') == 'Urine' ? 'selected' : '' }}>Tes Urine</option>
                            <option value="X-Ray" {{ old('jenis_pemeriksaan') == 'X-Ray' ? 'selected' : '' }}>Rontgen (X-Ray)</option>
                            <option value="USG" {{ old('jenis_pemeriksaan') == 'USG' ? 'selected' : '' }}>USG</option>
                            <option value="Parasit Kulit" {{ old('jenis_pemeriksaan') == 'Parasit Kulit' ? 'selected' : '' }}>Cek Parasit Kulit</option>
                            <option value="Lainnya" {{ old('jenis_pemeriksaan') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        <span class="error-message text-sm hidden" id="error-jenis_pemeriksaan" style="color: #ef4444;">Pilih jenis pemeriksaan</span>
                    </div>

                    <div class="mb-4">
                        <label for="keluhan_atau_alasan" class="block text-sm font-medium text-gray-700 mb-2">Alasan Pemeriksaan / Keluhan</label>
                        <textarea id="keluhan_atau_alasan" name="keluhan_atau_alasan" rows="4" 
                            placeholder="Contoh: Hewan muntah-muntah, atau cek rutin"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg resize-none placeholder-gray-500">{{ old('keluhan_atau_alasan') }}</textarea>
                        <span class="error-message text-sm hidden" id="error-keluhan_atau_alasan" style="color: #ef4444;">Alasan pemeriksaan harus diisi</span>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    <div class="flex items-center gap-2 mb-6">
                        ...
                        <h3 class="text-lg font-semibold">Tanggal Rencana Datang</h3>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <span class="error-message text-sm hidden" id="error-tanggal_booking"
                            style="color: #ef4444;">Tanggal harus diisi</span>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('bookingForm').addEventListener('submit', function(e) {
            ...
            let isValid = true;
            const fields = ['hewan_id', 'jenis_pemeriksaan', 'keluhan_atau_alasan', 'tanggal_booking'];
